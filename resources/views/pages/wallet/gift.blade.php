...
class GiftCardIndex extends Component
{
    ...
    {
        if (!$this->selectedCreditValue) return null;
    ...
    {
        if (!$this->selectedCreditValue) return false;
        ...
        try {
            DB::transaction(function () {
                ...
                $giftCard = GiftCard::create([
                    ...
                    'status' => 'active',
                    'expires_at' => now()->addYear(), // Expira em 1 ano
                ]);
                ...
                $message = \App\Services\NotificationMessages::giftCardPurchased(
                    $giftCard->code,
                    $this->selectedCreditValue
                );
    ...
    public function redeemGiftCard(): void
    {
        ...
        try {
            DB::transaction(function () {
                ...
                // Creditar na wallet
                $wallet->credit(
                    $giftCard->credit_value,
                    "Gift Card resgatado: {$giftCard->code}",
                    $giftCard
                );
                ...
                // Notificação Push
                $pushService = app(\App\Services\PushNotificationService::class);
                $message = \App\Services\NotificationMessages::giftCardRedeemed(
                    $giftCard->code,
                    $giftCard->credit_value
                );
...
<div class="min-h-screen bg-[#05070a] text-slate-200 pb-24 selection:bg-blue-500/30">
    <x-loading target="redeemGiftCard, purchaseGiftCard" message="PROCESSANDO..." />

    {{-- Efeitos de Fundo --}}
    <div class="fixed top-0 left-0 w-full h-full overflow-hidden -z-10 pointer-events-none">
        <div class="absolute top-[-10%] right-[-10%] w-[500px] h-[500px] bg-purple-600/10 blur-[120px] rounded-full"></div>
        <div class="absolute bottom-[-10%] left-[-10%] w-[400px] h-[400px] bg-pink-500/5 blur-[100px] rounded-full"></div>
    </div>
    ...
    <div class="max-w-6xl mx-auto px-6 pt-16">
        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-8">
            <div class="space-y-2">
                <div class="flex items-center gap-3">
                    ...
                    <span class="text-[10px] font-black text-purple-500 uppercase tracking-[0.5em] italic">Presentes Digitais</span>
                </div>
            ...
            <div class="flex items-center gap-6 bg-white/[0.02] border border-white/5 p-4 rounded-3xl backdrop-blur-md shadow-2xl">
                <div class="text-right">
                    <p class="text-[9px] font-black text-slate-500 uppercase tracking-widest italic leading-none mb-1">Seu Saldo</p>
                    <p class="text-2xl font-black text-white italic tracking-tighter">C$ {{ number_format($this->walletBalance, 0, ',', '.') }}</p>
                </div>
                <div class="w-12 h-12 bg-purple-500/10 border border-purple-500/20 rounded-2xl flex items-center justify-center text-xl shadow-lg">
                    💰
                </div>
            </div>
        </div>
        ...
        @if($tab === 'redeem')
            <div class="relative mb-24 group">
                <div class="absolute -inset-0.5 bg-gradient-to-r from-purple-600/50 to-pink-500/50 rounded-[3.5rem] blur opacity-20"></div>
                
                <div class="relative bg-[#0b0d11] border border-white/5 rounded-[3.5rem] p-16 shadow-2xl">
                    <div class="max-w-2xl mx-auto text-center space-y-8">
                        <div class="w-24 h-24 bg-purple-500/10 border border-purple-500/20 rounded-[2.5rem] mx-auto flex items-center justify-center text-5xl shadow-inner mb-8">
                            🎁
                        </div>
                        
                        <h2 class="text-3xl font-black text-white uppercase italic tracking-tighter">Resgatar Gift Card</h2>
                        <p class="text-slate-400 text-sm">Digite o código de 12 caracteres do seu Gift Card</p>

                        <div class="space-y-6">
                            <input 
                                type="text" 
                                wire:model.defer="redeemCode" 
                        ...
                        <div class="pt-8 border-t border-white/5">
                            <p class="text-[10px] text-slate-500 uppercase tracking-widest italic">💡 O crédito será adicionado automaticamente à sua carteira</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        @if($tab === 'purchase')
            <div class="mb-16">
                <div class="flex items-center gap-4 mb-12 px-2">
                    ...
                    <h2 class="text-sm font-black text-white uppercase tracking-[0.3em] italic">Escolha o Valor do Gift Card</h2>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                    @foreach($availableValues as $value)
                        <div 
                            wire:click="selectValue({{ $value }})"
                            ...
                            <div class="absolute inset-0 bg-gradient-to-b from-purple-600/[0.03] to-transparent opacity-0 group-hover:opacity-100 transition-opacity rounded-[3rem]"></div>

                            <div class="relative z-10 flex flex-col items-center">
                                <div class="w-20 h-20 bg-white/[0.02] border border-white/5 rounded-2xl mb-8 flex items-center justify-center text-3xl shadow-inner group-hover:scale-110 transition-transform">
                                    🎁
                                </div>

                                <div class="flex items-baseline gap-2 mb-8">
                                    <span class="text-5xl font-black text-white italic tracking-tighter">{{ number_format($value, 0, ',', '.') }}</span>
                                ...
                                <div class="w-full pt-8 border-t border-white/5 flex flex-col items-center gap-4">
                                    ...
                                    <div class="text-2xl font-black text-white italic">R$ {{ number_format($value, 2, ',', '.') }}</div>
                                    
                                    <button class="w-full h-14 bg-white/[0.03] border border-white/5 group-hover:bg-purple-600 group-hover:border-purple-500 rounded-2xl text-[10px] font-black uppercase tracking-[0.4em] text-white transition-all shadow-xl italic">
                                        COMPRAR
                                    </button>
                                </div>
                            </div>
                            ...
                            <div class="bg-white/[0.02] border border-white/5 rounded-2xl p-6 hover:bg-white/[0.04] transition-all">
                                ...
                                <div class="grid grid-cols-2 gap-4 text-xs">
                                    ...
                                    <div>
                                        ...
                                        <span class="text-white font-black ml-2">{{ $card->created_at->format('d/m/Y') }}</span>
                                    </div>
                                </div>
    ...
    @if($showPurchaseConfirmation && $selectedCreditValue)
        <div class="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            <div class="relative bg-[#0b0d11] w-full max-w-lg rounded-[4rem] border border-purple-600/30 overflow-hidden shadow-[0_0_150px_rgba(168,85,247,0.2)] animate-in zoom-in duration-300">
                <div class="h-1.5 bg-gradient-to-r from-purple-600 to-pink-400"></div>

                <div class="p-16">
                    <div class="text-center mb-12">
                        <div class="w-24 h-24 bg-purple-600/10 border border-purple-500/30 rounded-[2.5rem] mx-auto flex items-center justify-center text-5xl mb-8">
                            🎁
                        </div>
                        <h3 class="text-3xl font-black text-white uppercase italic tracking-tighter">Confirmar <span class="text-purple-500">Compra</span></h3>
                    </div>
                    ...
                    <div class="bg-white/[0.02] border border-white/5 rounded-[3rem] p-8 mb-10 space-y-6">
                        <div class="flex justify-between items-center border-b border-white/5 pb-6">
                            <span class="text-[9px] font-black text-slate-500 uppercase tracking-widest italic">Valor do Gift Card</span>
                            <span class="text-3xl font-black text-purple-500 italic">{{ number_format($selectedCreditValue, 0) }} C$</span>
                        </div>
                        <div class="flex justify-between items-center pt-2">
                            <span class="text-[9px] font-black text-purple-400 uppercase tracking-widest italic">Total a Pagar</span>
                            <span class="text-4xl font-black text-white italic tracking-tighter">R$ {{ number_format($this->selectedPrice, 2, ',', '.') }}</span>
                        </div>
                    </div>

                    <div class="space-y-4">
                        <button 
                            wire:click="purchaseGiftCard"
                            {{ $this->canPurchase ? '' : 'disabled' }}
