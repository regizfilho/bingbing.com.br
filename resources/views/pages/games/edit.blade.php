...
new class extends Component {
    ...
    {
        return in_array($this->game->status, ['draft', 'waiting']);
    }
        ...
        <div class="flex flex-col md:flex-row justify-between items-end gap-6 mb-12">
            <div>
                <div class="flex items-center gap-4 mb-3">
                    ...
                    <span class="text-blue-500 font-black tracking-[0.4em] uppercase text-[9px] italic">Painel de
                        Controle</span>
                </div>
                ...
                <p class="text-slate-500 text-sm font-bold mt-3 uppercase tracking-widest">Ajuste as configurações e
                    prêmios da sua partida</p>
            </div>

            <div class="flex items-center gap-4">
                <div class="bg-[#1c2128] px-6 py-4 rounded-xl border border-white/5">
                    <span class="text-[10px] font-black text-slate-500 uppercase italic tracking-widest block">Status da
                        Sala</span>
                    <span
                        class="text-sm font-black uppercase italic {{ $game->status === 'draft' ? 'text-slate-500' : 'text-amber-500' }}">
                        {{ $game->status === 'draft' ? 'RASCUNHO' : 'AGUARDANDO JOGADORES' }}
                    </span>
                </div>
                <a href="{{ route('games.index') }}"
                    class="p-4 bg-white/5 hover:bg-blue-600 border border-white/10 rounded-2xl transition-all group">
        ...
        <div class="space-y-10">
            ...
            <div class="bg-[#161920] border border-white/5 rounded-[2.5rem] p-10">
                <div class="flex items-center gap-3 mb-6">
                    ...
                    <span class="text-slate-500 text-[10px] font-black uppercase tracking-[0.3em] italic">Configurações
                        do Pacote</span>
                </div>
                ...
                <div class="flex flex-col md:flex-row md:items-end justify-between gap-8">
                    <div>
                        <h3 class="text-3xl font-black text-white uppercase italic tracking-tighter">
                            {{ $game->package->name ?? 'Padrão' }}</h3>
                        <p class="text-[9px] font-bold text-slate-600 uppercase tracking-widest mt-1 italic">Definido na
                            criação da sala</p>
                    </div>
                    ...
                    <div class="grid grid-cols-3 gap-8 border-l border-white/5 pl-8">
                        <div class="flex flex-col">
                            <span
                                class="text-[8px] font-black text-slate-500 uppercase tracking-widest mb-2 italic">Capacidade</span>
                            <span
                                class="text-sm font-black text-white uppercase italic tracking-tighter">{{ $game->package->max_players ?? '?' }}
                                Pessoas</span>
                        </div>
                        <div class="flex flex-col border-l border-white/5 pl-8">
                            <span
                                class="text-[8px] font-black text-slate-500 uppercase tracking-widest mb-2 italic">Cartela</span>
                            <span
                                class="text-sm font-black text-white uppercase italic tracking-tighter">{{ $game->card_size }}
                                Números</span>
                        </div>
                        <div class="flex flex-col border-l border-white/5 pl-8">
                            <span
                                class="text-[8px] font-black text-slate-500 uppercase tracking-widest mb-2 italic">Limite</span>
                            <span
                                class="text-sm font-black text-white uppercase italic tracking-tighter">{{ $game->max_rounds }}
                                Rodadas</span>
                        </div>
                    </div>
                </div>
            ...
            <form wire:submit.prevent="update" class="space-y-10">
                {{-- NOME DA SALA --}}
                <div class="bg-[#161920] border border-white/5 rounded-[2.5rem] p-10">
                    <label class="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2 block italic">1.
                        Identificação da Sala</label>
                    <p class="text-[9px] text-slate-600 font-bold mb-4 italic">Escolha o nome que os jogadores verão na
                        lista</p>
                    ...
                    @error('name')
                        <span
                            class="text-red-500 text-[10px] font-black uppercase mt-2 block italic">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div class="lg:col-span-2 bg-[#161920] border border-white/5 rounded-[3rem] p-10 space-y-10">
                        <div class="flex items-center gap-4 border-b border-white/5 pb-6">
                            ...
                            <div>
                                <h3 class="text-xs font-black text-white uppercase tracking-widest italic">Regras de
                                    Jogo</h3>
                                <p class="text-[8px] text-slate-600 font-bold mt-1 uppercase italic">Personalize a
                                    experiência dos jogadores</p>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div class="space-y-4">
                                <div>
                                    <label
                                        class="text-[9px] font-black text-slate-500 uppercase tracking-widest italic">Prêmios
                                        por Rodada</label>
                                    <p class="text-[8px] text-slate-700 font-bold mt-1 italic">Total disponível:
                                        {{ count($prizes) }}</p>
                                </div>
                            ...
                            <div class="space-y-4">
                                <div>
                                    <label
                                        class="text-[9px] font-black text-slate-500 uppercase tracking-widest italic">Cartelas
                                        por Jogador</label>
                                    <p class="text-[8px] text-slate-700 font-bold mt-1 italic">Quantidade por
                                        participante</p>
                                </div>
                                <div class="flex items-center gap-2">
                                    <input type="number" wire:model.live="cards_per_player" min="1"
                                        max="{{ $this->maxCardsPerPlayer }}"
                                        class="w-full bg-[#0b0d11] border border-white/10 rounded-2xl py-4 text-center font-black text-white text-xl focus:border-blue-500 outline-none">
                                    <span
                                        class="text-[8px] font-black text-slate-600 uppercase tracking-widest whitespace-nowrap">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            @foreach ([['m' => 'show_drawn_to_players', 't' => 'Ver Sorteados', 'd' => 'Exibir números sorteados'], ['m' => 'show_player_matches', 't' => 'Marcar Acertos', 'd' => 'Auto-destaque na cartela'], ['m' => 'auto_claim_prizes', 't' => 'Ganho Automático', 'd' => 'Bater automaticamente']] as $toggle)
                                ...
                                {{ $this->{$toggle['m']} ? 'bg-blue-600/10 border-blue-500 text-white' : 'bg-[#0b0d11] border-white/5 text-slate-500' }}">
                                    <div class="text-left">
                                        <span
                                            class="text-[10px] font-black uppercase italic block">{{ $toggle['t'] }}</span>
                                        <span
                                            class="text-[7px] text-slate-600 font-bold uppercase italic">{{ $toggle['d'] }}</span>
                                    </div>
                                    <div
                                        class="w-4 h-4 rounded {{ $this->{$toggle['m']} ? 'bg-blue-500' : 'bg-slate-800' }}">
                                    </div>
                                </button>
                            @endforeach
                        </div>
                    ...
                    <div class="space-y-8">
                        <div class="bg-[#161920] border border-white/5 rounded-[3rem] p-10">
                            <div>
                                <label
                                    class="text-[10px] font-black text-white uppercase tracking-widest mb-2 block italic">⚡
                                    Sorteio</label>
                                <p class="text-[8px] text-slate-600 font-bold mb-6 italic">Controle manual ou automático
                                </p>
                            </div>
                            ...
                            @if ($draw_mode === 'automatic')
                                <div class="pt-6 border-t border-white/5">
                                    <label
                                        class="text-[9px] font-black text-slate-500 uppercase tracking-widest italic mb-4 block">Tempo
                                        (Segundos)</label>
                                    <input type="number" wire:model.live="auto_draw_seconds" min="2"
                                        max="15"
                        ...
                        <div class="bg-[#161920] border border-white/5 rounded-[3rem] p-10">
                            <div>
                                <label
                                    class="text-[10px] font-black text-white uppercase tracking-widest mb-2 block italic">📏
                                    Cartela</label>
                                <p class="text-[8px] text-slate-600 font-bold mb-6 italic">Tamanho definido pelo pacote
                                </p>
                            </div>
                            <div
                                class="py-8 rounded-2xl border border-white/10 bg-[#0b0d11] flex flex-col items-center opacity-50">
                    ...
                    <div class="lg:col-span-3 bg-[#161920] border border-white/5 rounded-[3rem] p-10">
                        <div class="flex justify-between items-start mb-8">
                            <div class="flex items-start gap-4">
                                ...
                                <div>
                                    <h3 class="text-xs font-black text-white uppercase tracking-widest italic">Prêmios
                                        da Sala</h3>
                                    <p class="text-[8px] text-slate-600 font-bold mt-1 italic uppercase">Cadastre os
                                        prêmios que serão distribuídos</p>
                                </div>
                            </div>
                                ...
                                <div wire:key="prize-{{ $prize['temp_id'] }}"
                                    ...
                                    <div class="flex justify-between items-center">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span class="text-[9px] font-black text-slate-500 uppercase italic">Slot
                                                #{{ $index + 1 }}</span>
                                        </div>
                                        @if (count($prizes) > 1)
                                            <button type="button" wire:click="removePrize({{ $index }})"
                                                class="text-red-900 hover:text-red-500 transition-all">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 pt-10">
                    ...
                    @else
                        <button type="button" disabled
                            class="w-full py-8 rounded-[2.5rem] font-black uppercase text-xl tracking-[0.3em] italic bg-white/5 text-slate-800 border border-white/5">
                            SALA EM JOGO
                        </button>
                    @endif
